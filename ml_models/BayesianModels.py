...
class FixedParamLinearRegression(torch.nn.Module):
    """!! Note this class is for evaluation only, not training. It takes as input 
    a pre-trained BayesianLinearRegression() model and the guide. It creates a NN where the parameters are 
    fixed at the medians from this trained BNN. Can be used as a purely deterministic NN (_RETURN) 
    or to capture aleatoric uncertainly only, without epistemic (obs) """
    def __init__(self, model, guide):
        super().__init__()
        ...
        # Linear Regression, set parameters deterministically according to guide
        self.linear = torch.nn.Linear(self.n_features, self.n_targets)
        self.linear.weight = torch.nn.parameter.Parameter(guide.median()['linear.weight'])
        self.linear.bias = torch.nn.parameter.Parameter(guide.median()['linear.bias'])
