...
def bayesian_train_mcmc(params, training_params, model_name, model, kernel_name="NUTS"):
    """Learn posterior distribution of parameters using MCMC directly. User must specify kernel 
    for sampling next step."""
    ...
    for it in range(num_samples):
        params_it = param_sample(it, samples)
        conditioned_model = poutine.condition(model, params_it)
    ...
    plt.savefig(f"{save_model_path}/mcmc_{kernel_name}_loglikelihood.png")
    print(f"{save_model_path}/mcmc_{kernel_name}_loglikelihood_.png")
...
if __name__ == "__main__":
    ...
    model_name =  f"BayesianNN_Heteroscedastic_16_16_N{N_train}_priorNormal"      # Choose LinearRegression or NN 
